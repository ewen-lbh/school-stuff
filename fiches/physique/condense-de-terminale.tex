...
% Flow chart tikzstyles
\tikzstyle{decision} = [diamond, draw, fill=white, 
    text width=10em, text centered, node distance=1.75cm, inner sep=0pt, minimum height=4em]
...
\begin{description}
    \item[\chemfig{O < H}] Devant
    \item[\chemfig{O <: H}] Derrière
\end{description}

\begin{figure}[h]
    \centering
    \chemfig{H -[6] (<:[5] H_3C) (<[5.5] Cl) (-[7] OH)}
...
\begin{description}
    \item[\chemfig{C-C}] Rotation possible et n'affecte pas
\end{description}
...
Objet non-superposable à son image mirroir $\iff$ Objet chiral

\[\text{achiral} := \overline{\text{chiral}}\]
...
\begin{equation*}
\begin{split}
    \chemfig{C (-A) (-[2] A) (-[4] A) (-[6] A)} &\iff \text{C est symétrique}\\
    \overline{\text{C est symétrique}} &\iff \text{C est asymétrique} \\
				       &\iff \chemfig{^\ast C} \quad \text{(notation)}
\end{split}
\end{equation*}

\subsection{Classification des stéréoisomères}

\emph{Pour les questions, on se demande si $A$ et $B$, deux molécules isomères, peuvent être indentiques en faisant subir à l'une une transformation.}

\begin{figure}[h]
\centering
\begin{tikzpicture}[node distance = 4cm, auto]
	\node [decision] (simples) {rotations autour de liaisons \emph{simples}?};
	\node [block, below right of=simples] (confor) {de conformation};
	\node [block, below of=confor] (ident) {indentiques};
	\node [block, below left of=simples] (config) {de configuration};
	\node [decision, below of=config] (mirroir) {image dans un mirroir plan?};
	\node [block, below left of=mirroir] (dias) {diastéréoisomères};
	\node [block, below right of=mirroir] (enan) {énantiomères};
	\path [line] (simples) -- (config) node [near start] {non};
	\path [line] (simples) -- (confor) node [near start] {oui};
	\path [line] (confor) -- (ident);
	\path [line] (config) -- (mirroir);
	\path [line] (mirroir) -- (enan) node [near start] {oui};
	\path [line] (mirroir) -- (dias) node [near start] {non};
\end{tikzpicture}
\label{fig:classification_stereoisomeres}
\end{figure}

\subsubsection{Différences de propriétés selon le type de stéréoisomère}
