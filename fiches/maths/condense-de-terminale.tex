...
\subsection{Équations de cercle $(x - a)^2 + (y - b)^2 = R^2$}

Soit $a, b, R$ des réels, $R$ étant le rayon du cercle.\\
...
\begin{table}[H]
    ...
    \begin{tabular}{r||l|c|c|c|l}
        ...
        $\displaystyle\lim_{n\to+\infty} q^n$ & ? & \threeCols{0} & $+\infty$ \\\hline
...
\begin{table}[H]
    ...
    \begin{tabular}{l|lll|l}
        ...
        Théorème des gendarmes    & $w_n \geq v_n \geq u_n$  & $l_u = l_w = 0$  & $\implies l_v = L$    & ($u_n$) et ($w_n$) forcent ($v_n$) à tendre vers $L$\\
    \end{tabular}
\end{table}
\newpage
...
\begin{equation*}
    \begin{split}
        P(A|B)\ou P_B(A) &= \frac{P(P \cap Q)}{P(B)}\quad\text{si}\:P(B)\ne 0\\
    \end{split}
\end{equation*}


\subsection{Probabilités d'intersections $P(P \cap Q)$}
Probabilité que $A$ \textbf{et} $B$ soit réalisées.

\begin{equation*}
    \begin{split}
        P(P \cap Q) &= P(B)\cdot P(A|B)\\
                    &= P(A)\cdot P(B|A)
    \end{split}
\end{equation*}

\subsection{Probabilités d'union $P(A \cup B)$}
L'union $\cup$ c'est un "ou exclusif":

$$P(A \cup B) = P(A) + P(B) + P(P \cup Q)$$
...
\begin{itemize}
    ...
    \item L'union de chaque partition couvre l'univers entier \\$\iff \bigcap_{i=1}^j B_i = \Omega$
\end{itemize}
...
Soit $x$, $C$ et $D$ des nombres et $\Psi$ un nombre ou symbole

\begin{equation*}
    \begin{split}
        \lim_{x \to \Psi} C = D &\iff C \goesto{x}{\Psi} D \\
        &\iff \text{$C = D$ quand $x = \Psi$}
    \end{split}
\end{equation*}
...
Une fonction est continue quand "on peut tracer sa courbe sans lever le stylo". Plus rigoureusement, la fonction $f$ est continue sur l'intervalle $I$ si, pour tout $a \in I$, $f(a) \goesto{x}{a} a$.
...
\text{$f$ est strictement monotone sur $I$}\\
k \in [f(a); f(b)]\quad(1)
\end{cases}$$
$$
...
\noindent(1) si $a$ ou $b = \pm\oo$, on calcule la limite pour l'intervalle image:\\
\textit{Montrer que $f(x) = k$ n'admet qu'une seule solution dans $\R$}
$$k \in \left]\limminfty f(x); \liminfty f(x)\right[$$

\vspace{20pt}

\noindent\textit{Attention}: pour dire que $f(x) = k$ on n'utilise pas la bijection mais le tableau de variations
...
$\square$ représente les opérations $+$, $\times$ et $\div$

\begin{itemize}
    \item $z\cdot\conj{z} &= (\im\; z)^2 + (\re\; z)^2$
    \item $\conj{z \:\square\: w} &= \conj{z} \:\square\: \conj{w}$
...
L'affixe est un point représentant un nombre complexe dans le plan:

$$\Aff(a+ib) = \begin{pmatrix} a\\b \end{pmatrix}$$
...
$$S = 
\begin{cases}
\Delta < 0 &\iff \left\{ \frac{-b+i\sqrt{-\Delta}}{2a}; \conj{S_1}  \right\} \\
\Delta = 0 &\iff \left\{ \frac{-b}{2a} \right\} \\
\Delta > 0 &\iff \left\{ \frac{-b+\sqrt{\Delta}}{2a}; \frac{-b-\sqrt{\Delta}}{2a}\right\}
\end{cases}
$$
...
Soit $u$ et $v$ des fonctions dérivables

\begin{equation*}
    \begin{split}
        (u^n)' &= nu'u^{n-1}\quad n\in\Z\cup\{0.5\}
    \end{split}
\end{equation*}
...
\textit{$[AB]$ sur une face, $C$ sur face adjaçente}\\
