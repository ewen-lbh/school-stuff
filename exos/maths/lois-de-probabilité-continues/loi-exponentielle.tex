...
\title{Lois de probabilités: exponentielle}
...
\section{Vidéo 1: Exercice}

\begin{equation*}
    \begin{split}
        \lim_{k\to +\infty}\int_0^k f(x)dx &= \lim_{k\to+\infty}\int_0^k \lambda e^{-\lambda x} \\
        &= \lim_{k\to +\infty} \lambda \left[ \frac{e^{-\lambda x}}{-\lambda} \right]_0^k
    \end{split}
\end{equation*}
